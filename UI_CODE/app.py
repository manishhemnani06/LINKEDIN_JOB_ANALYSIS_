    ...
    for i in skills:
        ...
        if (i in skills):
            ...
            if st.button('Look all job postings >'):
                st.dataframe(data[['Company_Name', 'Class', 'Designation', 'Location', 'Total_applicants',
        ...
        ###Members
        if i in ['ADITYA','ADITYA MALAV']:
            title_container = st.container()
            col1, mid, col2 = st.columns([1, 8, 32])
            with col1:
        ...
        if i in ['GREESHMA','GREESHMA TOM']:
            ...
            with col1:
                g = Image.open('g.png')
                st.image( g, width=150)
            with col2:
                st.write(f"""#####  Sorry,[{i}](https://www.linkedin.com/in/greeshma-tom-454a57237/) is not a skill but She is the member of this project. who did this project in very well manner............☺️""")
                break

        if i in ['MANISH','MANISH HEMNANI']:
            title_container = st.container()
            col1, mid, col2 = st.columns([1, 8, 32])
            with col1:
                m = Image.open('m.png')
                st.image( m, width=150)
            with col2:
                st.write(f"""#####  Sorry, [ {i}](https://www.linkedin.com/in/manish-hemnani-280953179/) is not a skill but He is the member of this project. who did this project in very well manner............☺️""")
                break

        if i in ['SOHAL','MOHAMMAD SOHAL']:
            title_container = st.container()
            col1, mid, col2 = st.columns([1, 8, 32])
            with col1:
                s = Image.open('s.jpeg')
            ...
            with col2:
                st.write(f"""#####  Sorry, [ {i}](https://www.linkedin.com/in/mohammad-sohal-5b8b38224/) is not a skill but He is the member of this project. who did this project in very well manner............☺️""")
                break

        if i in ['SUSHANT','SUSHANT KUMAR']:
            title_container = st.container()
            col1, mid, col2 = st.columns([1, 8, 32])
            with col1:
                a = Image.open('shusant.png')
                st.image( a, width=150)
            with col2:
                st.write(f"""#####  Sorry, [ {i}](https://www.linkedin.com/in/sushant-kumar-lkdn/) is not a skill but He is a Data Analyst in Masai School. Who is Highly skilled, competent and well-experienced data analyst with organized.............☺️""")
                break
